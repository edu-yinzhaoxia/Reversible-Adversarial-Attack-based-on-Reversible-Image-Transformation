...
%%%% Pxy Qxy x->y
L=length(img(:))/2;%Ã¿´ÎÇ¶Ò»°ë
waterMark = img;
[ht,wd] = size(img);

preV = zeros(ht*wd/2,1);
difs = zeros(ht*wd/2,1);
xpos = zeros(ht*wd/2,1);
ypos = zeros(ht*wd/2,1);
...
overLen = 8000;
[mod_diffs,messLen,nLast] = modifyDiffs(difs(1:pfor-overLen),bpp,L);
psnr=appraise_PSNR(mod_diffs,difs(1:pfor-overLen))
